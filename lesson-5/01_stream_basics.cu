...
int main(void) {
    ...
    vectorAdd<<<blockPerGrid, threadPerBlock, 0, stream1>>>(d_A, d_B, d_C, numElements);

    // Copy results back to host async
    ...
    for (int i = 0; i < numElements; i++) {
        if (fabs(h_A[i] + h_B[i] - h_C[i]) > 1e-5) {
            fprintf(stderr, "Results verification failed %d!\n", i);
            exit(EXIT_FAILURE);
        }
    }
