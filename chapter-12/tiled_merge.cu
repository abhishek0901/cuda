...
__global__ void tiled_merge_gpu(int *A, int *B, int *C, int size_A, int size_B, int tile_size) {
    ...
    if (threadIdx.x == 0) {
        A_S[0] = co_rank(C_curr, A, size_A, B, size_B);
        A_S[1] = co_rank(C_next, A, size_A, B, size_B);
    }
    __syncthreads();
    int A_curr = A_S[0];
    int A_next = A_S[1];
    ...
    while (counter < total_iteration) {
       ...
       int a_curr = co_rank(c_curr, 
        ...
        merge_seq(A_S + a_curr, B_S + b_curr, C + C_curr + C_completed + c_curr, a_next - a_curr, b_next - b_curr);
        counter++;
...
int main() {
    ...
    tiled_merge_gpu<<<dim_grid, dim_block, sharedMemSize>>>(A_d, B_d, C_d, size_A, size_B,tile_size);
